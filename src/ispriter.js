//增加特性  1. 支持md5 使用6位字符串还是32位呢？ 支持配置化
//		  2. 0 0问题替换成top left  支持配置化
// 		  3. cache化， 支持图片任务缓存，如果所有输入的图片文件没有变化，不产生图片合并 直接使用已合并文件
...
module.exports = function(conf, done) {
	...
	var DEFAULT_CONFIG = {
			...
			"replace00":true,//替换 background 00 成top left
	...
	var ori_md5 = require('MD5');

	function md5(data, fileName) {
		var str = ori_md5(data);
		var name = fileName.substr(fileName.lastIndexOf("/") + 1);
		name = name.substr(0, name.lastIndexOf("."));
	
		var len=(spriteConfig&&spriteConfig.md5len)||6;//默认6位MD5 重复的可能性还是有的  @@@@todo
	...
	var BaseCSSStyleDeclaration = {
			...
			mergeBackgound: function() {
				...
				var positionText = this.removeProperty('background-position-x') + ' ' + this.removeProperty('background-position-y');
				if(positionText==="0 0"&&spriteConfig&&spriteConfig.replace00){
					console.log("############","replace00");
					positionText="top left";
				}
				style.setProperty('background-position', positionText.trim(), null);

				var toMergeAttrs = ['background-color', 'background-image', 'background-position', 'background-repeat', 'background-attachment', 'background-origin', 'background-clip'];
				for (var i = 0, item; item = toMergeAttrs[i]; i++) {
						if (style.hasOwnProperty(item)) {
							background += this.removeProperty(item) + ' ';
						}
					}
				var mergedBg=background.trim();
				style.setProperty('background', mergedBg, null);
			},
	...
	function collectStyleRules(styleSheet, result, styleSheetUrl) {
			...
			styleSheet.cssRules.forEach(function(rule, i) {
				...
				if (regexp.ignorePosition.test(style['background-position-x']) || regexp.ignorePosition.test(style['background-position-y'])) {
				...
				if (regexp.ignoreRepeat.test(style['background-repeat']) || regexp.ignoreRepeat.test(style['background-repeat-x']) || regexp.ignoreRepeat.test(style['background-repeat-y'])) {
	...
	function positionImages(styleObjList) {
			...
			for (var i in styleObjList) {
					...
					if (styleObj && styleObj.imageInfo && styleObj.imageInfo.drew) {
	...
	function drawImageAndPositionBackground(spriteTask, callback) {
			...
			var combinedCssRules, spriteArray = spriteTask.spriteArray;
			...
			if (!spriteArray[spriteArray.length - 1].root) {
				...
				styleObjArr.forEach(function(styleObj) {
					...
					// 修改 background 属性
					replaceAndPositionBackground(imageName, styleObj, combinedCssRules[imageName]);
				});
			}

			spriteArray.forEach(function(styleObjArr, i) {
				var png, imageName, imageName2, // 用于输出 log 
				imageAbsName;

				png = createPng(styleObjArr.root.w, styleObjArr.root.h);

				imageName = createSpriteImageName(spriteTask.cssFileName, i, spriteArray.length);
				...
				styleObjArr.forEach(function(styleObj) {
					...
					// 对图片进行填充
					image.bitblt(png, 0, 0, image.width, image.height, imageInfo.fit.x, imageInfo.fit.y);
				...
				if (styleObjArr.length) { // 没必要输出一张空白图片
					...
					//stream.write(data);
					//stream.flush();
					//console.log("write stream to buff");
					//console.log("data.length:",data.length);
					var pngMD5 = imageName.substr(0, imageName.lastIndexOf("/") + 1) + md5(data, imageName) + ".png";
					imgNameMapping[imageName] = pngMD5;
					//console.log("imageName=",imageName);
					//console.log("md5Name=",pngMD5);
					imageName2 = path.join(spriteConfig.output.cssDist, pngMD5);
					...
					//此处动刀子  millerliu 2014-5-20


					stream.pipe(fs.createWriteStream(imageAbsName)).on('finish', function() {
					...
					styleObjArr.forEach(function(styleObj) {
						...
						// 延迟修改 background 属性
						replaceAndPositionBackground(pngMD5, styleObj, combinedCssRules[imageName]);
	...
	function replaceAndPositionBackground(imageName, styleObj, combinedSelectors) {
			//console.log("replace bg img:",imageName);
			styleObj.cssRules.forEach(function(style) {
	...
	function combineSpriteTasks(spriteTaskArray) {
			...
			var combinedStyleSheetArray = [],
				...
				combinedFileName, combinedSpriteTask;
	...
	function exportCssFile(spriteTask) {
			...
			// 把合并了的样式统一在一起输出到文件的最后  不建议这样做  可以考虑放在最前  也不建议  不改变现有文件
			if (spriteConfig.output.combineCSSRule) {
					combinedCssRules = spriteTask.combinedCssRules;
					for (imageName in combinedCssRules) {
						var mapName = imgNameMapping[imageName];
						//console.log("combined css rules imageName",imageName,"map name",mapName);
						cssContent += combinedCssRules[imageName].join(',') + '{' + 'background-image: url(' + mapName + ');' + '}\n';
					}
			...
			//var cssbeautify=require("cssbeautify");//格式化一次样式
			var beautified = cssbeautify(cssContent, {
					//indent: '  ',
					"indent_size": 4,
			...
			nf.writeFileSync(fileName, beautified, true);
	...
	function styleSheetToString(styleSheet) {
			...
			for (var i = 0; i < rules.length; i++) {
					...
					//console.log("rule["+i+"]=["+rule.cssText+"]");
					//console.log(rule);
	...
	function copyUnspriteCss(spriteTask) {
			var fileName, fileName2, // 用于输出 log
			cssContent;
	...
	function merge(config, done) {
			...
			zTool.forEach(spriteConfig.input.cssSource, function(cssFileName, i, next) { // onEach
				...
				var spriteTask = { // an SpriteTask, 一个文件一个 SpriteTask
					...
					// css 文件的路径
					styleSheet: readStyleSheet(cssFileName),
					// css 文件的内容
					styleObjList: null,
					// 搜集到的需要合并图片的样式和相关图片信息(大小宽高等)
