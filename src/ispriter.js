...
//                    combinedCssRules[imageName]);
        ...
        if(styleObjArr.length){ // 没必要输出一张空白图片
			...
			var pngMD5=imageName.substr(0,imageName.lastIndexOf("/")+1)+md5(data)+".png";
			...
			stream.pipe(fs.createWriteStream(imageAbsName))
					.on('finish', function(){
			...
			styleObjArr.forEach(function(styleObj){
				...
				// 延迟修改 background 属性
				replaceAndPositionBackground(pngMD5, styleObj, 
						combinedCssRules[imageName]);
